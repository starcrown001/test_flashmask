...
def generate_qk_sparse_mask( maskout_pair=[(1024, 538), (2358, 1700)],total_seqlen=8192) -> tuple[list[list[int]], list[list[int]], list[bool]]:
    ...
    for i in range(len(offsets)):
        ...
        if mask_offset != 0:
            ...
            q_ranges.append([offset, total_seqlen])
            k_ranges.append([0, offset + mask_offset])
            is_causal_mapping.append(False)
...
def generate_random_eviction_mask(start_row = 4096, total_seqlen = 8192):
    ...
    q_ranges += [[int(start_rows[id]), total_seqlen] for id in range(S)]
    k_ranges += [[id, id+1] for id in range(S)]
    is_causal_mapping += [True for _ in range(S)]
        ...
        for D in [64, 128, 256]:
            ...
            for idx, (S, prefix_doc_seq_lens, qksparse_mask) in enumerate(doc_seq_lens_list):
                ...
                available_examples = {
                    ...
                    "Share Question Mask": lambda: test_mask(mask_mod=generate_share_question_mask(doc_seq_lens=share_qa_docs), B=B, S=S, H=H, D=D, dtype=dtype),
                    "Global Sliding Window": lambda: test_mask(mask_mod=generate_global_sliding_window_mask(global_token=16, window_size=int(S*0.0625), total_seqlen = S), B=B, S=S, H=H, D=D, dtype=dtype, disable_fwd_atomic_reduction = True),
