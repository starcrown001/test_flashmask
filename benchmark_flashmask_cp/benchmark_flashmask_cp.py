...
args = get_args()
output_prefix = "flashmask_ag"
folder_suffix = "_ag_413"
if args.overlap:
    output_prefix = "flashmask_long"
    folder_suffix = "_overlap_only"
if args.batch == 1:
...
def do_bench_flashmaskcp(q_local, k_local, v_local, o_grad_local, startend_row_indices, group, is_causal, warmup=WARM_UP, rep=BENCH_TIME, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
    """
    ...
    # Bind the attention function once — no branch in the hot loop
    if args.overlap:
        attn_fn = partial(overlap_flashmask_attention, q_local, k_local, v_local, startend_row_indices, causal=is_causal, use_rs=args.use_rs)
        mode_str = "overlap"
    else:
        attn_fn = partial(flashmask_attention_cp, q_local, k_local, v_local, startend_row_indices, causal=is_causal)
        mode_str = "baseline"
    ...
    n_repeat = max(5, rep)
    print(f"Record info ({mode_str}). profile: {args.profile}. warmup: {WARM_UP}, rep: {BENCH_TIME}. Actual warmup: {n_warmup}, rep: {n_repeat}")
    # Warm-up
...
def main(examples: List[str] = ["all"], dtype='bf16', batch: int = 1, num_heads=8, overlap=False, use_rs=False, profile=False):
    ...
    with open(input_file, 'r') as f:
        ...
        for D in [128]:
            for idx, (S, prefix_doc_seq_lens, qksparse_mask) in enumerate(doc_seq_lens_list):
                ...
                os.makedirs(f"{dtype}_dist_test{folder_suffix}", exist_ok=True)
                text_file = open(f"{dtype}_dist_test{folder_suffix}/{output_prefix}_{B}_{S}_{H}_{D}_{idx}_{rank}.csv","w")
