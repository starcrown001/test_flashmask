...
# FlashMask CP Benchmark — Intra-Machine Balance (Strategy 3)
# Uses cp_balance_comm_new.balance_flashmask_input_inter_machine with use_intra_machine_balance=True
# Usage: bash run_benchmark_intra_balance.sh [MODE]
...
ARGS="--mode $MODE --use_rs --use_locality_swap --use_intra_machine_balance"
PYTHON_BIN="/root/paddlejob/share-storage/gpfs/system-public/xiehaoyang/xhy_backup/fm_test_env/bin/python"
BENCH_FILE="benchmark_flashmaskcp_intra_balance.py"

echo "Start benching mode=$MODE with: $PYTHON_BIN $BENCH_FILE $ARGS"
...
for num_head in "${num_heads[@]}"; do
    ...
    $PYTHON_BIN -m paddle.distributed.launch \
        --log_dir paddle_cp_logs_intra_balance/output_$rank/ \
        --master $master:$port \
        --nnodes $nnodes \
        --rank $rank \
        --run_mode=collective \
        $BENCH_FILE --cp_size 32 --num_heads $num_head $ARGS --epsilon 0.3
done
echo "End benching mode=$MODE with: $PYTHON_BIN $BENCH_FILE $ARGS"
