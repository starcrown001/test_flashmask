...
def do_bench_flashmaskcp(q_local, k_local, v_local, o_grad_local, startend_row_indices, group, is_causal, mode = "balance_q", warmup=WARM_UP, rep=BENCH_TIME, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
    ...
    n_repeat = max(5, rep)
    print(f"Record info (balance + overlap). profile: {args.profile}. warmup: {WARM_UP}, rep: {BENCH_TIME}. Actual warmup: {n_warmup}, rep: {n_repeat}. Use RS: {args.use_rs}")
    # Warm-up
...
def cp_flashmask_balance_bench(query, key, value, startend_row_indices, is_causal,o_grad,mode):
    ...
    # a2a_o = lambda: balance_alltoall(local_o_grad, cp_size, cp_group, balance_q_chunksize, recv_dict, send_dict)
    # a2a_x_time = do_bench_dist(a2a_o,cp_group = cp_group)
        ...
        for H in [args.num_heads]:
            for D in [128]:
                for idx, (S, prefix_doc_seq_lens, qksparse_mask) in enumerate(doc_seq_lens_list):
                    ...
                    text_file = open(f"{dtype}_dist_test/{output_prefix}_{B}_{S}_{H}_{D}_{idx}_{rank}.csv","w")
