#!/usr/bin/env python3
"""
Unified FlashMask CP Benchmark
...
Supports 4 modes via --mode:
  ...
  balance:          balanced_swap + flashmask_attention_cp (balance_q)
  balance_overlap:  balanced_swap + overlap_flashmask_attention (balance_q)
...
# ---------------------------------------------------------------------------
def get_args():
    parser = ArgumentParser(description="Unified FlashMask CP Benchmark")
...
# ---------------------------------------------------------------------------
output_prefix = f"flashmask_unified_{args.mode}"
...
def prepare_inputs(query, key, value, startend_row_indices,
    ...
    if mode in ["baseline", "overlap"]:
        ...
        # K/V: Uniform splitting (consistent with old benchmark;
        # only affects output values, not performance or mask correctness)
...
def main():
    ...
    print(f"[Rank {rank}] Unified FlashMask CP Benchmark — mode={args.mode}, "
    ...
    for H in [args.num_heads]:
        for D in [128]:
            for idx, (S, prefix_doc_seq_lens, qksparse_mask) in enumerate(doc_seq_lens_list):
                ...
                out_dir = f"{args.dtype}_dist_test"
