...
def get_args():
    ...
    parser.add_argument("--plot_computation", action="store_true",
                        help="Generate per-rank local computation bar chart")
...
def compute_rank_computation(mask_np, block_size):
    """Count effective (non-masked) KV blocks for one rank's local mask.

    A block is effective if any element has start > end.
    Returns the total count of such blocks.
    """
    S = mask_np.shape[2]
    block_size = 128
...
def plot_computation_bar(before_comp, after_comp, ranks, cp_size,
                         block_size, gpus_per_node, idx, mask_name, out_dir):
    """Plot per-rank local computation (effective KV block count) as a bar chart.

    If before_comp is provided, draws grouped bars (before vs after);
    otherwise only draws after bars.
    """
    n_ranks = len(ranks)
    x = np.arange(n_ranks)
    bar_width = 0.35

    fig, ax = plt.subplots(figsize=(max(8, n_ranks * 0.4), 5), facecolor='white')

    has_before = len(before_comp) > 0
    if has_before:
        before_vals = [before_comp[r] for r in ranks]
        after_vals = [after_comp[r] for r in ranks]
        bars_b = ax.bar(x - bar_width / 2, before_vals, bar_width,
                        label='Before', color='#90CAF9', edgecolor='#1565C0', linewidth=0.5)
        bars_a = ax.bar(x + bar_width / 2, after_vals, bar_width,
                        label='After', color='#EF9A9A', edgecolor='#C62828', linewidth=0.5)
    else:
        after_vals = [after_comp[r] for r in ranks]
        bars_a = ax.bar(x, after_vals, bar_width,
                        label='After', color='#EF9A9A', edgecolor='#C62828', linewidth=0.5)

    # Node boundary lines
    n_nodes = (cp_size + gpus_per_node - 1) // gpus_per_node
    for nd in range(1, n_nodes):
        bx = nd * gpus_per_node - 0.5
        if bx < n_ranks:
            ax.axvline(bx, color='#C62828', linewidth=1.0, linestyle='--', alpha=0.4)

    # Mean line(s)
    after_mean = np.mean(after_vals)
    ax.axhline(after_mean, color='#C62828', linewidth=1.0, linestyle=':', alpha=0.6,
               label=f'After mean={after_mean:.1f}')
    if has_before:
        before_mean = np.mean(before_vals)
        ax.axhline(before_mean, color='#1565C0', linewidth=1.0, linestyle=':', alpha=0.6,
                   label=f'Before mean={before_mean:.1f}')

    ax.set_xlabel('Rank', fontsize=11)
    ax.set_ylabel('Effective KV Blocks', fontsize=11)
    ax.set_title(
        f'Local Computation per Rank\n'
        f'idx={idx}  mask={mask_name}  CP={cp_size}  block={block_size}',
    ...
    out_path = os.path.join(out_dir, f'computation_bar_idx{idx}_{mask_name}.png')
    fig.savefig(out_path, dpi=180, bbox_inches='tight', facecolor='white')
    print(f"Saved: {out_path}")
    plt.close(fig)
...
def main():
    ...
    for (idx, mask_name) in sorted(groups.keys()):
        ...
        # -- Local computation (effective KV blocks per rank) --
        if args.plot_computation:
            after_comp = {}
            for rank in ranks:
                after_comp[rank] = compute_rank_computation(
                    after_masks[rank], args.block_size)

            before_comp = {}
            if before_masks:
                for rank in ranks:
                    before_comp[rank] = compute_rank_computation(
                        before_masks[rank], args.block_size)

            # Table
            comp_rows = []
            for rank in ranks:
                bc = before_comp.get(rank, "N/A")
                ac = after_comp[rank]
                delta = ac - bc if isinstance(bc, (int, float)) else "N/A"
                comp_rows.append([rank, bc, ac, delta])
            print(f"\n--- Local Computation: Effective KV Blocks (idx={idx}, mask={mask_name}) ---")
            print(tabulate(comp_rows,
                           headers=["Rank", "Before Blocks", "After Blocks", "Delta"],
                           tablefmt=tfmt, floatfmt=".0f"))

            # Aggregate
            comp_agg_rows = []
            after_comp_s = stats(np.array([after_comp[r] for r in ranks], dtype=float))
            if before_comp:
                before_comp_s = stats(np.array([before_comp[r] for r in ranks], dtype=float))
                for key in ["Total", "Mean", "Max", "Min", "Max-Min", "Std"]:
                    b, a = before_comp_s[key], after_comp_s[key]
                    pct = (a - b) / b * 100 if b != 0 else float('nan')
                    comp_agg_rows.append([key, b, a, f"{pct:+.2f}%"])
            else:
                for key in ["Total", "Mean", "Max", "Min", "Max-Min", "Std"]:
                    comp_agg_rows.append([key, "N/A", after_comp_s[key], "N/A"])
            print(tabulate(comp_agg_rows,
                           headers=["Metric", "Before", "After", "Δ%"],
                           tablefmt=tfmt, floatfmt=".2f"))

            # Bar chart
            plot_computation_bar(before_comp, after_comp, ranks, cp_size,
                                 args.block_size, args.gpus_per_node,
                                 idx, mask_name, args.mask_dir)
