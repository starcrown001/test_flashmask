...
from flash_mask.cp_balance import balance_flashmask_input, get_q_workload, assign_tasks_ipo, assign_tasks_heap
...
args = get_args()
output_prefix = "flashmask_balance"
if args.batch == 1:
...
def do_bench_flashmaskcp(q_local, k_local, v_local, o_grad_local, startend_row_indices, group, is_causal, mode = "balance_q", warmup=WARM_UP, rep=BENCH_TIME, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
    """
    Benchmark the runtime of the provided function. By default, return the median runtime of :code:`fn` along with
    the 20-th and 80-th performance percentile.
    :param fn: Function to benchmark
    ...
    print(f"wsm debug {q_local.shape=}, {k_local.shape=}, {v_local.shape=}, {startend_row_indices.shape=}")
    out_local = flashmask_attention_cp(q_local, k_local, v_local, startend_row_indices, causal=is_causal, mode=mode)
    # print('pt00')
    ...
    n_repeat = max(5, rep)
    print(f"Record info (balance only). profile: {args.profile}. warmup: {WARM_UP}, rep: {BENCH_TIME}. Actual warmup: {n_warmup}, rep: {n_repeat}. Use IPO: {cp_use_ipo}")
    # Warm-up
...
def cp_flashmask_balance_bench(query, key, value, startend_row_indices, is_causal,o_grad,mode):
    ...
    local_startend_row_indices, buckets = balance_flashmask_input(startend_row_indices, cp_size, rank, use_ipo=cp_use_ipo)
    local_q = scatter_balance(query, group = cp_group, axis=1,mode = "balanced_swap", buckets = buckets).detach().contiguous()
    print("pass0")
    balancex = lambda: balance_flashmask_input(startend_row_indices, cp_size, rank, use_ipo=cp_use_ipo)
    ...
    local_startend_row_indices, buckets = balance_flashmask_input( startend_row_indices, cp_size, rank,balance_chunk_size= balance_q_chunksize, use_ipo=cp_use_ipo)
...
def test_cp_famask(
    ...
    return fwd_time, bwd_time, total_time, balance_time,scatter_x_time,gather_x_time
...
def main(examples: List[str] = ["all"], dtype='bf16', profile=False, batch=1, num_heads=8):
                ...
                for idx, (S, prefix_doc_seq_lens, qksparse_mask) in enumerate(doc_seq_lens_list):
                    ...
                    for ex in ex_to_run:
                        if ex in available_examples:
                            print(ex)
                            fw_time, bw_time, total_time, balance_time, scatter_x_time, gather_x_time = available_examples[ex]()
                    ...
                    os.makedirs(f"{dtype}_dist_test_balance", exist_ok=True)
                    text_file = open(f"{dtype}_dist_test_balance/{output_prefix}_{B}_{S}_{H}_{D}_{idx}_{rank}.csv","w")
                    text_file.write(content2)
                    text_file.close()
                    # assert False

if __name__ == "__main__":
    print("New FlashMask Balance (no overlap)!")
    main(**vars(args))
