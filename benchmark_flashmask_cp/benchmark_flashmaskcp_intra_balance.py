#!/usr/bin/env python3
"""
FlashMask CP Benchmark — Inter-Machine Comm-Aware Balance + Intra-Machine Balance (Strategy 3)

Uses balance_flashmask_input_inter_machine from cp_balance_comm_new (with use_intra_machine_balance).
...
# Add current directory to path for cp_balance_comm_new import
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from context_parallel_utils_new import scatter_balance, flashmask_attention_cp
from cp_balance_comm_new import balance_flashmask_input_inter_machine
...
# ---------------------------------------------------------------------------
def get_args():
    parser = ArgumentParser(description="FlashMask CP Benchmark — Intra-Machine Balance (Strategy 3)")
    ...
    parser.add_argument("--use_intra_machine_balance", action="store_true",
                        help="Enable intra-machine load balancing (Strategy 3): "
                             "re-distribute tasks within each machine using assign_tasks_heap "
                             "for pure compute balancing after locality swap.")
...
# ---------------------------------------------------------------------------
output_prefix = f"flashmask_intra_balance_{args.mode}"
...
def do_bench_flashmaskcp_cpu(attn_fn, cp_group, warmup=WARM_UP, rep=BENCH_TIME,
                              profile=False, rank=0, fast_flush=True):
    """
    Benchmark using CPU timer (barrier + sync + perf_counter).

    Same structure as do_bench_flashmaskcp but uses wall-clock timing
    with barrier+synchronize at each timing point.
    """
    # Dry run
    out = attn_fn()
    paddle.distributed.barrier(group=cp_group)
    ...
    print(f"Warmup completed.{cp_group.rank}")
    ...
    # fwd_time = sum(times_fwd) / rep
    # bwd_time = sum(times_bwd) / rep
...
def prepare_inputs(query, key, value, startend_row_indices,
    ...
    - balance/balance_overlap: Q/K/V via balanced_swap, local mask
      (uses balance_flashmask_input_inter_machine with use_intra_machine_balance)
    """
    if mode in ["baseline", "overlap"]:
        ...
        local_mask, buckets = balance_flashmask_input_inter_machine(
            ...
            use_intra_machine_balance=args.use_intra_machine_balance,
            use_ipo=use_ipo)
...
def test_cp_flashmask(generate_mask_fn, B=1, S=8192, H=1, D=128, dtype='bf16'):
    ...
    # Benchmark
    fwd_time, bwd_time = do_bench_flashmaskcp_cpu(
        attn_fn, cp_group,
...
def main():
    ...
    print(f"[Rank {rank}] FlashMask CP Benchmark (Intra-Machine Balance — Strategy 3) — mode={args.mode}, "
          ...
          f"use_intra_machine_balance={args.use_intra_machine_balance}")
    ...
    for H in [args.num_heads]:
        for D in [128]:
            for idx, (S, prefix_doc_seq_lens, qksparse_mask) in enumerate(doc_seq_lens_list):
                ...
                out_dir = f"{args.dtype}_dist_test_{args.mode}_locswap_{args.use_locality_swap}_" \
                          f"intrabalance_{args.use_intra_machine_balance}_" \
                          f"eps_{args.epsilon}_{FIXED_LOCAL_LENGTH}"
