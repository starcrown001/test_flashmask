...
def do_bench_flashmaskcp(q_local, k_local, v_local, o_grad_local, startend_row_indices, group, is_causal, mode = "balance", warmup=WARM_UP, rep=BENCH_TIME, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
    """
    Benchmark the runtime of the provided function. By default, return the median runtime of :code:`fn` along with
    the 20-th and 80-th performance percentile.
    :param fn: Function to benchmark
    ...
    n_repeat = max(5, rep)
    print(f"Record info (balance + overlap). profile: {args.profile}. warmup: {WARM_UP}, rep: {BENCH_TIME}. Actual warmup: {n_warmup}, rep: {n_repeat}. Use RS: {args.use_rs}. Use IPO: {cp_use_ipo}")
    # Warm-up
