...
def run_te_dist_attn(
    ...
    Uses CPU timing (time.perf_counter()) with proper synchronization.
    """
    ...
    for i in range(n_repeat):
        ...
        # Backward timing with CPU timer
        out.backward(dout_local, retain_graph=True)
        torch.cuda.synchronize()
        dist.barrier()
    ...
    dist.all_reduce(fwd_times_tensor, op=dist.ReduceOp.MAX, group=dist.group.WORLD)
    dist.all_reduce(bwd_times_tensor, op=dist.ReduceOp.MAX, group=dist.group.WORLD)
...
def main(examples: List[str] = ["all"], dtype='bf16', attn_impl='ring'):
    """Run the TE CP attention benchmark with CPU timing.
    ...
    for H in [1,4,8]:
        ...
        for idx, (S, prefix_doc_seq_lens, qksparse_mask) in enumerate(doc_seq_lens_list):
            ...
            for impl in impls_to_run:
                ...
                out_dir = f"{dtype}_te_{impl}_cpu_test"
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Run TE CP attention benchmark with CPU timing for document masks.")
    parser.add_argument(
