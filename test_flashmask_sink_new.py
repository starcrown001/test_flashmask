...
import paddle
from paddlefleet.transformer.sink_impl_attnsink_new import sink_attention_forward
...
def test_flashmask(
    batch_size, seqlen_q, seqlen_k, nheads, nheads_kv, d, dv, nheads_startend_row_indices, use_sink, fa_version, dtype, gen_startend_row_indices, softcap=0.0
):
    paddle.seed(2024)
    assert nheads % nheads_kv == 0

    # This file validates FA3 attention sink support through sink_attention_forward.
    if not use_sink:
        pytest.skip("sink_attention_forward requires a learnable sink")
    if d != dv:
        pytest.skip("FA3 attention sink wrapper currently requires D == Dv")
    ...
    # learnable_sink: per q-head sink logit (bf16, shape [nheads]). It is an
    # un-scaled logit competing in the softmax denominator (not multiplied by
    # softmax_scale), matching the fa4 cute kernel semantics.
    if use_sink:
    ...
    if use_sink:
        ...
        # dsink_atol = 2 * (sink_ref.grad + 0.3 - 0.3 - sink_ref.grad).abs().max().item() + (0 if softcap == 0 else 3e-4)

        # assert (sink.grad - sink_ref.grad).abs().max().item() <= rtol * (sink_bf16.grad - sink_ref.grad).abs().max().item() + dsink_atol
        # bf16 round-trip 量化误差：1 ULP 上界 = 2^-7 * |dsink|
        dsink_quant = 2**-7 * sink_ref.grad.abs().max().item()
        dsink_atol = (
            2 * (sink_ref.grad + 0.3 - 0.3 - sink_ref.grad).abs().max().item()
            + dsink_quant
            + (0 if softcap == 0 else 3e-4)
        )
        assert (sink.grad - sink_ref.grad).abs().max().item() \
            <= rtol * (sink_bf16.grad - sink_ref.grad).abs().max().item() + dsink_atol