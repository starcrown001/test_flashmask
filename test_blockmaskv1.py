...
def tensor_md5(tensor):
    ...
    print(hashlib.md5(x_bytes).hexdigest())
...
def from_paddle(x: paddle.Tensor):
    ...
    elif x.dtype == paddle.int32 or x.dtype == "int32":
      return torch.from_numpy(x.numpy()).to("cuda").type(torch.int32)
    else:
      assert False
...
def test_mask_with_v1(
    ...
    value = paddle.randn([batch_size, total_k, num_head, head_size], dtype=paddle.bfloat16)
    o_grad = paddle.randn([batch_size, total_q, num_head, head_size], dtype=paddle.bfloat16)
    query = paddle.from_numpy(np.load("/root/paddlejob/workspace/env_run/lousiyu/XFlex/output/torch_results/xattn_v14_thr0p95_stride8/64k/layer_04/q_u16.npy")).view(paddle.bfloat16)
    key = paddle.from_numpy(np.load("/root/paddlejob/workspace/env_run/lousiyu/XFlex/output/torch_results/xattn_v14_thr0p95_stride8/64k/layer_04/k_u16.npy")).view(paddle.bfloat16)
    value = paddle.from_numpy(np.load("/root/paddlejob/workspace/env_run/lousiyu/XFlex/output/torch_results/xattn_v14_thr0p95_stride8/64k/layer_04/v_u16.npy")).view(paddle.bfloat16)
    # 
    ...
    print(query.shape)
    print(key.shape)
    batch_size, total_q, num_head, head_size = query.shape
    _,total_k,_,_ = key.shape
    o_grad = paddle.randn([batch_size, total_q, num_head, head_size], dtype=paddle.bfloat16)
    ...
    blockmask = paddle.from_numpy(np.load("/root/paddlejob/workspace/env_run/lousiyu/XFlex/output/torch_results/xattn_v14_thr0p95_stride8/64k/layer_04/block_mask.npy")).to(paddle.int32)
    # blockmask = blockmask[:,15:16,:,:]
    ...
    print(key1.shape)
    print(value1.shape)
    print(startend_row_indices1.shape)
    # blockmask[0,0,251,0] = 0
    ...
    # rrattn_startend_row_indices = paddle.from_numpy(np.load("/root/paddlejob/workspace/env_run/lousiyu/XFlex/output/torch_results/xattn_v14_thr0p95_stride8/64k/layer_04/startend_row_indices.npy"))
    # tensor_md5(rrattn_startend_row_indices)
    # tensor_md5(startend_row_indices1)
    
    # rrattn_blockout = paddle.from_numpy(np.load("/root/paddlejob/workspace/env_run/lousiyu/XFlex/output/torch_results/xattn_v14_thr0p95_stride8/64k/layer_04/blockmask_out.npy")).view(paddle.bfloat16)
    
    # strict_check(rrattn_blockout, out1)
    ...
    # paddle.device.synchronize()
    # max_diff = paddle.abs(ref_out1 - out1)
    # coords = paddle.nonzero(max_diff == paddle.max(max_diff))[0]
    # print(coords) 
    # print(blockmask[0,0,251,:])
    # strict_check(ref_out1[:,251 * 128:252 * 128,:,:].flatten(), out1[:,251 * 128:252 * 128,:,:].flatten())
    
    # ref_out1.backward(o_grad)
    # print(q1.grad[0,0,:,0])
    
    torch_q1 = from_paddle(q1).reshape(-1, num_head, head_size)
    torch_k1 = from_paddle(k1).reshape(-1, num_head, head_size)
    torch_v1 = from_paddle(v1).reshape(-1, num_head, head_size)
    torch_block_mask = from_paddle(blockmask)
    
    cu_seqlens = torch.arange(0, (batch_size + 1) * total_q, step=total_q, dtype=torch.int32).to("cuda:0")
    head_mask_type = torch.tensor([1] * num_head, dtype=torch.int32).to("cuda:0")
    ...
    block_attention_out = block_sparse_attn_func(torch_q1, torch_k1, torch_v1, cu_seqlens, cu_seqlens, head_mask_type, None, torch_block_mask, total_q, total_q, 0, is_causal=causal, exact_streaming=False)
    
    strict_check(out1.flatten(), block_attention_out.flatten())
