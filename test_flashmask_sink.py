...
# from paddlefleet.transformer.sink_impl_new import sink_attention_forward
...
def test_flashmask(
    ...
    # learnable_sink (attention sink) is only implemented on the fa4 / cute path.
    # if use_sink and fa_version != 4:
    #     pytest.skip("learnable_sink is only supported on fa_version=4 (cute) path")
    ...
    if use_sink:
        ...
        dsink_atol = 2 * (sink_ref.grad + 0.3 - 0.3 - sink_ref.grad).abs().max().item()
