...
def test_mask_with_v1(
    ...
    total_q = cu_seqlens_q[-1] 
    total_k = cu_seqlens_k[-1] 
    query = paddle.randn([batch_size, total_q, num_head * 4, head_size], dtype=paddle.bfloat16) 
    key = paddle.randn([batch_size, total_k, num_head, head_size], dtype=paddle.bfloat16)
    value = paddle.randn([batch_size, total_k, num_head, head_size], dtype=paddle.bfloat16)
    o_grad = paddle.randn([batch_size, total_q, num_head * 4, head_size], dtype=paddle.bfloat16)
    ...
    # ref_out1 = attention_naive_with_mask_varlen(q1, k1, v1, flashmask_to_densemask(startend_row_indices,paddle.bfloat16, causal),cu_seqlens_k)
    # paddle.device.synchronize()
...
def main(examples: List[str] = ["all"]):
    ...
    params_list = [
        ...
        # },
        # {
        #     'cu_seqlens_q': list([0,384]),
        #     'cu_seqlens_k': list([0,384]),
        #     'num_head': int(8),
        #     'head_size': int(128),
        #     'batch_size': int(8),
        ...
        #     # "Causal Blockwise Mask": {'doc_seq_lens': [2048, 2048, 4096]},
        #     # "Prefix LM Document Mask": {'doc_seq_lens': [(1024, 2538), (1742, 1742), (1146, 3912)]},
        #     # "Prefix LM Causal Mask": {'prefix_length': 1024},
        #     # "QK-sparse Mask": {'maskout_pair': [(2048, 1023), (4096, 2048)]},
        #     "Random Eviction Mask": {"start_row": 4096},
        # },
        # {
